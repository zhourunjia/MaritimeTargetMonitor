...
$distRoot = Join-Path $repoRoot "dist\MaritimeTargetMonitor_Offline"
...
if (Test-Path $edgeSrc) {
    $edgeDst = Join-Path $distRoot "edgeyolo-main"
    New-Item -ItemType Directory -Force -Path $edgeDst | Out-Null
    robocopy $edgeSrc $edgeDst /E /XD .venv __pycache__ .git .idea .vscode /XF *.pyc *.pyo *.log | Out-Null
}
...
# Ensure sqlite3.dll in root if available from runtime
$sqliteSrc = Join-Path $repoRoot "runtime\python\sqlite3.dll"
if (Test-Path $sqliteSrc) {
    Copy-Item -Force $sqliteSrc (Join-Path $distRoot "sqlite3.dll")
}

# Create zip package
$zipPath = Join-Path $repoRoot "dist\MaritimeTargetMonitor_Offline_Package.zip"
if (Test-Path $zipPath) { Remove-Item -Force $zipPath }
Compress-Archive -Path (Join-Path $distRoot '*') -DestinationPath $zipPath -Force

# Build IExpress SED
$sedPath = Join-Path $repoRoot "dist\MaritimeTargetMonitor_Offline_Setup.sed"
$setupExe = Join-Path $repoRoot "dist\MaritimeTargetMonitor_Offline_Setup.exe"
...
$lines.Add('FriendlyName=MaritimeTargetMonitor Offline')
