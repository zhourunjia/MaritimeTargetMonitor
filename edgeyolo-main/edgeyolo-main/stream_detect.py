...
def parse_args():
    parser = argparse.ArgumentParser("EdgeYOLO stream detect")
...
def _open_ffmpeg_reader_with_retry(source: str, ffmpeg_path: str, out_size, input_size, retries: int, interval: float):
    ...
    if (width is None or height is None) and out_size is not None:
        width, height = out_size
    if (width is None or height is None) and input_size is not None:
...
def _build_ffmpeg_cmd(ffmpeg, width, height, fps, output):
    ...
    if lower.startswith("rtsp://") or lower.startswith("rtsps://"):
        ...
        # Let ffmpeg infer format by extension
        base += [output]
    return base
...
def _open_video_writer(path, width, height, fps):
...
def main():
    ...
    use_ffmpeg = _is_stream_url(args.output)
    ffmpeg_proc = None
    writer = None
    ...
    if use_ffmpeg:
        ...
        writer = _open_video_writer(args.output, width, height, fps)
        if writer is None or not writer.isOpened():
            print(f"[error] failed to open output file: {args.output}")
            return 3
